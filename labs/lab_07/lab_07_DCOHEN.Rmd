...
```{r}
# the buyer county and buyer state are not showing up in either table, this could cause problems because we wold not be able to pinpoint where exactly the buyer of these pills are. 
```
...
ggplot(aes(fill = pills_per_person)) + 
    geom_sf(lwd = 0) + 
  theme_map() + 
labs(fill='Pills per Person in 2009',title="Peak rates of opioid-related fatalities in the Appalachian region", subtitle = "WV and KY had highest opioid death rates per person in 2009", caption = "Source; arcos database") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person from 2006-2012 increased mainly in the appalachian region", subtitle = "Kentucky and 
       West Virginia saw the largest spikes from 2006-2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
geo_pills_population_county_maryland %>%
  ...
  geom_sf(lwd = 0) + 
  theme_map() + 
  labs(fill='pills per person in each county',title="The trend spiked from 2006-2012", subtitle = "Amount of pills shipped to Maryland counties 2006-2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
```
