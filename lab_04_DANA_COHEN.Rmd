...
mingo_working <- mingo %>%
  mutate(state_dsitribution = if_else(reporter_state > "WV",
"in_state_distribution", "out_of_state_distribution"))                                
```    
...
```{r}
mingo %>%
  group_by(buyer_name, reporter_name, combined_labeler_name) %>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit) 
            ) %>%
  arrange(desc(total_pills))

#Tug Valley, Miami-Luken, Actavis Pharma Inc. had 4954200 pills sent.
...
```{r}
mingo%>%
  group_by(reporter_name, reporter_dea_no, reporter_city, reporter_address1)%>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit),
            avg_pills_per_shipment = mean(dosage_unit),
           min_pills_in_shipment = min(dosage_unit),
...
```{r}
# first, I create a new column with the case 1 function named "cardinal health facilities" with the mutated function. In the parenthesis in the case 1 function, I would tell {r} to put the names of distributors with cardinal in their name while also specifying a second command to  make another column with all other reporters.
```
