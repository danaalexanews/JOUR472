...
#NUMBER OF PILLS FLOWING THROUGH BALTIMORE CITY
    ```{r}
...
```
#BALTIMORE DEATH RATE, MEDIAN INCOME
```{r}
...
#Q4 - What nonwhite-populations (similar to Baltimore) also have high opioid death rate? 
...
# B16010_015 = high school or equivalent
  # B16010_041 = bachelor's degree
  # B16010_028 = Estimate!!Total!!Some college or associate's degree
  # B16010_002 = Less than high school
  # B17018_025 = total income in past 12 months at or above poverty level
  # B17018_001 = poverty 
  # B17018_002 = income in last 12 months below poverty level

bach_degree <- get_acs(geography = "county", 
              variables = c(medincome = "B16010_041"),
              year = 2010,
              state = "MD")
hs_or_equivalent <-get_acs(geography = "county", 
              variables = c(medincome = "B16010_015"),
              year = 2010,
              state = "MD")

assoc_degree <- get_acs(geography = "county", 
              variables = c(medincome = "B16010_028"),
              year = 2010,
              state = "MD")

under_hs <- get_acs(geography = "county", 
              variables = c(medincome = "B16010_002"),
              year = 2010,
              state = "MD")

income_above_poverty_level <- get_acs(geography = "county", 
              variables = c(medincome = "B17018_025"),
              year = 2010,
              state = "MD")

poverty <- get_acs(geography = "county", 
              variables = c(medincome = "B17018_001"),
              year = 2010,
              state = "MD")

income_below_poverty_level <- get_acs(geography = "county", 
              variables = c(medincome = "B17018_002"),
              year = 2010,
              state = "MD")
...
bach_degree <- bach_degree %>%
  mutate(bach_degree = estimate) %>% 
  select(-variable, -moe, -estimate) %>%
  clean_names()

hs_or_equivalent <-hs_or_equivalent %>%
  mutate(hs_or_equivalent = estimate) %>%
  select(-variable, -moe, -estimate) %>%
  clean_names()

assoc_degree <- assoc_degree %>%
  mutate(assoc_degree = estimate) %>%
  select(-variable, -moe,-estimate) %>%
  clean_names()
  

under_hs <- under_hs %>%
mutate(under_hs = estimate) %>%
select(-variable, -moe,-estimate) %>%
  clean_names() 
  
income_above_poverty_level <- income_above_poverty_level %>%
  mutate(income_above_poverty_level = estimate) %>%
  select(-variable, -moe,-estimate) %>%
  clean_names()

poverty <-poverty %>%
  mutate(poverty = estimate) %>%
  select(-variable, -moe,-estimate) %>%
  clean_names()
  
income_below_poverty_level <- income_below_poverty_level %>%
  mutate(income_below_poverty_level = estimate) %>%
  select(-variable, -moe,-estimate) %>%
  clean_names()


save_data <- start_data %>%
  inner_join(second_data_frame, by = "the common column or columns")
  
md_edu_total <- bach_degree %>%
  inner_join(hs_or_equivalent,  by=c("name", "geoid")) %>%
  inner_join(assoc_degree, by=c("name", "geoid")) %>%
  inner_join(under_hs, by=c("name", "geoid")) %>%
  inner_join(income_above_poverty_level, by=c("name", "geoid")) %>%
  inner_join(poverty, by=c("name", "geoid")) %>%
  inner_join(income_below_poverty_level, by=c("name", "geoid"))

#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXx
tn_bach_degree <- get_acs(geography = "county", 
              variables = c(tn_bach_degree = "B16010_041"),
              year = 2010,
              state = "TN")
tn_hs_or_equivalent <-get_acs(geography = "county", 
              variables = c(tn_hs_or_equivalent = "B16010_015"),
              year = 2010,
              state = "TN")

tn_assoc_degree <- get_acs(geography = "county", 
              variables = c(tn_assoc_degree = "B16010_028"),
              year = 2010,
              state = "TN")

tn_under_hs <- get_acs(geography = "county", 
              variables = c(tn_under_hs = "B16010_002"),
              year = 2010,
              state = "TN")

tn_income_above_poverty_level <- get_acs(geography = "county", 
              variables = c(tn_income_above_poverty_level = "B17018_025"),
              year = 2010,
              state = "TN")

tn_poverty <- get_acs(geography = "county", 
              variables = c(tn_poverty = "B17018_001"),
              year = 2010,
              state = "TN")

tn_income_below_poverty_level <- get_acs(geography = "county", 
              variables = c(tn_income_below_poverty_level = "B17018_002"),
              year = 2010,
              state = "TN")
...
***next step is to interjoin tennesse stuff with marylad stuff 
...
#DEATH RATE V. HOUSEHOLD MEDIAN INCOME
```{r}
#Q5 - What is the correlation between death rates and Household Median Income on a National Scale?
#A5 - This has a considerably weak correlation. On a national level, the higher the median household income is, there becomes fewer death rates on average.

total_opiod_death_rate_income %>%
  ungroup()%>%
...
```{p}
#Q8 What about the Appalachia (Knotts County,TN)?
...
#CRIME RATE
```{r}
#Q9  - What is the crime rate in Baltimore in comparison on a National Level?

#A6 - 
```

```{r}
#Q10 -  What is the crime rate in PG County comapred on a national level?
...
```{p}
#Q11 - What is the crime rate in the Appalachia?
#A11
```
...
#EDUCATION
```{r}
#Q12 - Compared to nationally... where does Baltimore City rank in education?
```

```{r}
#Q13 - Compared to nationally... where does PG rank in education?
#A13 - 
```

```{r}
#Q14  Compared to nationally... where does Knotts Conty rank in education?
...
#POVERTY
```{r}
#Q15 - WHAT ARE BALTIMORE'S POVERTY LEVELS? 
#A15 - 
```

```{r}
#Q15 - WHAT ARE PG COUNTY'S POVERTY LEVELS? 
#A15 - 
```

```{r}
#Q15 - WHAT ARE KNOTTS COUNTY POVERTY LEVELS? 
#A15 - 
```
