...
#A3- Comparing Howard County and Baltimore city, no, that was not the case. Looking at the data, Howard County had less pills per person than Baltimore (23 pills to 30 pills), and HOCO's median household income is nearly triple the amount Baltimore City's is.
...
#A4 - Looking at Howard County and Baltimore City, no - this is not the case. Howard County's Median Household Income is well over 100k and holds the highest income of any county in Maryland. Baltimore however, has one of the lowest incomes (about 40k) and has 5x the death rate of Howard County.
...
ggplot(opiod_death_rate_income) +
  ...
  labs(x="Opioid Death Rate", y="Median Household Income", title="", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
```{r}
#Q5 - Do communities with higher death rates have higher median incomes on a NATIONAL SCALE?
#A5- Becuase HOCO and baltimore are outliers, they do not align themselves or are not indicative compared on a national scale.
...
```{r}
#Q6 -Do communities with a larger share of minorities have a higher opioid death rate? 
#A6- Though more nonwhite populations appear to have a higher death rate, Baltimore city is the outlier. 
...
ggplot(nonwhite_population_death_rate) +
  ...
  geom_label_repel(aes(age_adjusted_rate, pct_nonwhite_population, label=County),
                  subset(nonwhite_population_death_rate, County == "Howard County, MD" | County == "Baltimore city, MD" | County == "Cherokee County, OK" | County == "St. Louis city, MO")
                  )
...
#Q7 - What nonwhite-populations (similar to Baltimore) also have high opioid death rate? 
#A7 St. Louis, MO |  Cherokee County, OK | Columbus County, NC | Valencia County, NM
```{r}
...
```{r}
#Q8 - What is the correlation between death rates and Household Median Income on a National Scale?
#A8 - This has a considerably weak correlation. On a national level, the higher the median household income is, there becomes fewer death rates on average.
...
```{r}
#Q10 -  What is the crime rate in St. Louis, MO in comparison on a National Level?

#A10-
```

```{r}
#Q11- What is the crime rate of Cherokee County, OK in comparison to a national level?
```

AGE DEMOGRAPHIC
```{r}
#Q12 - In Baltimore city, what age group is the MOST effected by this epidemic
...
```{r}
#Q14 - In St. Louis, what age group is the MOST effected by this epidemic
...
```{r}
#Q16 - In St. Louis, what age group is the MOST effected by this epidemic
...
```{r}
#Q18 - Compared to nationally... where does Baltimore City rank in education?
```

```{r}
#Q17 Compared to nationally... where does St. Louis rank in education?

```

```{r}
#Q18 Comparedt to nationally... where does Cherokee County rank in education?
```
...
#Q13 - Looking at the state of Maryland, are counties with lower housing prices places where death rates were higher?
county_median_household_prices <- get_acs(geography = "county",
             variables = "B25105_001", geometry = FALSE)
```

```{r}
#Q14 - Looking at the state of Maryland, are counties with lower education levels places where death rates were higher?
county_education_business_science_jobs <- get_acs(geography = "county",
             variables = "B24011_010", geometry = FALSE)
```

```{r}
#Q15 -Looking at the state of Maryland, are counties with a high criminal record education levels places where death rates were higher?
...
```{r}
#Q17 -Which distributors were most responsible for flooding pills into Howard County at the height of the opioid crisis?
```

```{r}
#Q18 -Which distributors were most responsible for flooding pills into Baltimore City at the height of the opioid crisis?
```


```{r}
#Q19 -Purdue Pharma is often credited with/blamed for launching the opioid crisis. What do the flow of sales from that drug company look like from both Howard County and Baltimore City?
