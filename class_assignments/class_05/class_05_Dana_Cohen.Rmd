...
#I am loading the data that I will be using to help guide me through this lab.
```
...
# The number of records buyer_annual_by_year is higher than buyer_totals because the buyer_totals is the sum of pills from the buyer_dea_no. That number is higher becuase some pharmacies have added new entries to the list.

# Thebuyer_addresses is higher than buyer_totals because buyer_addresses only has the state and county of where the pills have come from. Buyer_total doesn't include that excerpt of information.
```
## Analysis
...
#The name of the pharmacy is VA CONSOLIDATED MAIL OUTPATIENT PHARMACY based in Charleston, SC.
```
...
buyer_annual_by_year %>%
  left_join(buyer_addresses, by="buyer_dea_no") %>%
  filter(year==2010, buyer_state.x == "MD", buyer_bus_act.x== "PRACTICIONER")%>%
  arrange(desc(dosage_unit))
  
# The name of the practicioner is Rakesh Mathur and had 337450 total pills. The next highest doctor had 212800 pills. Mathur is currently on probation. His pharmacy is located in Baltimore, MD 21218. His license number is D39170. HIs self-designated practice area is anesthesiology, internal medicine, and critical care medicine. I would ensure that the doctor you find on the state board lookup tool is the same doctor in this data set because the license numbers match.
```
...
buyer_totals %>%
  ...
  mutate(avg_pills_per_person = total_pills/population_average) %>%
  arrange(desc(avg_pills_per_person))  

 #West Virginia had the highest rate of total pills per person sent between 2006 to 2012.
...
buyer_totals%>%
  inner_join(buyer_addresses, by="buyer-dea-no")
#an inner join will match records between both data sets.
buyer_totals %>%
  left_join(buyer_addresses, by = "buyer-dea-no")
#a left join will match buyer totals along with buyer addreses that match all buyer total table times.
buyer_totals%>%
  left_join(buyer_addresses, by = "buyer-dea-no") %>%
  filter(is.na(buyer_address1))
  #the join function allows all totals to have addresses where they match. There are zero records returned, that means there are no null addresses.
buyer_totals%>%
  anti_join(buyer_addresses, by ="buyer_dea_no")
#an antijoin checks if there are any buyer addresses that do not match the records.
...
```{r}
buyer_adresses%>%
  inner_join(buyer_totals, by="buyer-dea-no")%>%
#an inner join returns all matching records in both records.
buyer_adresses%>%
  left_join(buyer_totals, by="buyer_dea_no")%>%
  # left join matches buyer totals along with buyer addreses that match all buyer total table times.
buyer_adresses%>%
  left_join(buyer_totals, by = "buyer_dea_no")%>%
  filter(is.na(total_pills))
# A left join will with the filter will use all the addresses and join all matching buyer totals while showing the buyers that don't have transactions in the totals table.
buyer_addresses %>%
  anti_join(buyer_totals, by="buyer_dea_no")
#an antijoin will do something similar and show any buyers that don't have any pills bought in the total table.
```
## Your own questions
